...
public class FulfillmentCenterOneProcessor {
	...
	public String transformToOrderRequestMessage(String orderXml) {
		...
		try {
			if (orderXml == null) {
				throw new Exception("Order xml not bound to method via integration framework.");
			}
			
			output = processCreateOrderRequestMessage(orderXml);
		} catch (Exception e) {
			log.error("Fulfillment center one message translation failde: "
					+ e.getMessage(), e);
		}
	...
	// OLLO PODE SER PROTECTED e pode lanzar Exception
	private String processCreateOrderRequestMessage(String orderXml) throws JAXBException {
		
		// 1 - Unmarshall the Order from an XML string to the generated order class.
		...
		com.pluralsight.orderfulfillment.generated.Order order = 
				(com.pluralsight.orderfulfillment.generated.Order) unmarshaller.unmarshal(new StringReader(orderXml));
	...
	private OrderRequest buildOrderRequestType(com.pluralsight.orderfulfillment.generated.Order orderFromXml) {
